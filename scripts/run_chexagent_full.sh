#!/bin/bash
# CheXAgent SAE 完整流水线（Steering + Monosemanticity Score）
# 与 run_llava_med_full.sh 配置相同，仅模型换为 chexagent（SigLIP ViT-L, layer=18）
...
RESULTS_DIR="${PROJECT_DIR}/results/chexagent_omnimed"
...
# ── Step 5: 编码图片嵌入（BiomedCLIP，可复用已有结果）──────────────────────
echo ""
